...
function Sidebar({
    ...
    return (
        <>
            ...
            <aside
                className={cn(
                    "fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-[#15316B] to-[#0E2450] shadow-[4px_0_32px_-4px_rgba(0,0,0,0.18)] transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 flex flex-col shrink-0",
            ...
            >
                ...
                <div className="h-16 flex items-center justify-between px-6 border-b border-white/[0.08] shrink-0">
                    ...
                    <button onClick={onClose} className="p-1 rounded-md text-white/50 hover:bg-white/10 lg:hidden">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-4 shrink-0">
                    <div className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl">
                        <div className="w-10 h-10 rounded-lg bg-[#F5B800] shadow-sm flex items-center justify-center shrink-0">
                            <GraduationCap className="w-5 h-5 text-[#15316B]" />
                        </div>
                        <div className="min-w-0 flex-1">
                            <p className="text-[10px] font-bold text-white/50 uppercase tracking-wider mb-0.5">Connected School</p>
                            <p className="text-xs font-bold text-white/90 truncate">{schoolName}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-6">
                    {NAV_GROUPS.map((group) => (
                        <div key={group.label}>
                            <h3 className="px-3 mb-2 text-[11px] font-bold tracking-wider text-white/50 uppercase">
                                {group.label}
                            </h3>
...
export function StudentLayout() {
    ...
    return (
        <div className="flex h-screen font-sans overflow-hidden text-slate-900" style={{ background: '#FBF9F5' }}>
            ...
            <div className="flex-1 flex flex-col min-w-0 relative">
                ...
                {/* Topbar */}
                <header className="h-16 bg-white/95 backdrop-blur-md border-b border-[#EEEAE0] flex items-center justify-between px-4 lg:px-8 shrink-0 z-10">
                ...
                {/* Page Content */}
                <main className="flex-1 overflow-auto p-4 sm:p-6 lg:p-8" style={{ background: '#FBF9F5' }}>
