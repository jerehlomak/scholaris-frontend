    ...
    return (
        ...
        >
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0E2450] via-[#122F69] to-indigo-900 p-6 text-white shadow-lg">
                    ...
                    <div className="relative flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-1 text-white/70 text-sm">{profile?.user?.email || 'Welcome to the parent portal'}</p>
                        </div>
                        <div className="hidden sm:flex items-center gap-2 shrink-0">
                            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 text-center border border-white/20">
                                ...
                                <p className="text-[11px] text-white/70 font-medium">Children</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <motion.div variants={itemVariants} className="h-full">
                    <Card className="h-full flex flex-col shadow-sm border-slate-200">
                        <CardHeader className="pb-3 border-b border-slate-100 flex flex-row items-center justify-between space-y-0">
                            ...
                            <Link to="/parent/children" className="text-xs font-semibold text-[#1E4DA6] hover:text-[#173F8C] transition-colors">
                                Manage
                            </Link>
                        </CardHeader>
                ...
                <motion.div variants={itemVariants} className="h-full flex flex-col">
                    <Card className="h-full flex flex-col shadow-sm border-slate-200">
                        <CardHeader className="pb-3 border-b border-slate-100 flex flex-row items-center justify-between space-y-0">
                            ...
                            <Link to="/parent/fees" className="text-xs font-semibold text-[#1E4DA6] hover:text-[#173F8C] transition-colors">
                                View Ledger
                            </Link>
                        </CardHeader>
                        <CardContent className="p-0 flex-1 flex flex-col">
                            ...
                            {recentFees && recentFees.length > 0 && (
                                <div className="p-4 border-t border-slate-100 bg-slate-50/50 text-right rounded-b-xl">
                                    <Link to="/parent/fees">
                                        <Button size="sm" className="bg-[#1E4DA6] hover:bg-[#173F8C] text-white font-semibold shadow-sm text-xs">
            ...
            {recentMessages.length > 0 && (
                <motion.div variants={itemVariants}>
                    <Card className="bg-white border-[#1E4DA6]/10 shadow-sm overflow-hidden">
                        <CardHeader className="bg-[#1E4DA6]/8 pb-4 border-b border-[#1E4DA6]/10 flex-row items-center justify-between">
                            <CardTitle className="text-lg font-bold text-slate-800 flex items-center gap-2">
                                <BellRing className="text-[#1E4DA6] w-5 h-5" />
                                Recent Reminders & Messages
                            </CardTitle>
                            <Link to="/parent/finance-messages" className="text-xs font-bold text-[#1E4DA6] hover:underline">View All</Link>
                        </CardHeader>
                        <CardContent className="p-0">
                            <div className="divide-y divide-slate-100">
                                {recentMessages.slice(0, 3).map((msg, idx) => (
                                    <div key={idx} className="p-4 flex gap-4 hover:bg-slate-50 transition-colors">
                                        <div className="w-10 h-10 rounded-full bg-[#1E4DA6]/10 flex items-center justify-center shrink-0">
                                            <MessageSquare className="w-5 h-5 text-[#1E4DA6]" />
                                        </div>
                                        <div>
