...
const AVATAR_COLORS = ['bg-blue-600', 'bg-pink-500', 'bg-emerald-500', 'bg-amber-500', 'bg-purple-500'];
...
export default function ParentChildren() {
    ...
    if (loading) return (
        <div className="flex items-center justify-center p-20 w-full">
            <Loader2 className="w-8 h-8 animate-spin text-[#0036a1]" />
        </div>
    );
    ...
    if (children.length === 0) return (
        <div className="max-w-[1200px] mx-auto w-full font-dash pb-10">
            <div className="p-20 text-center bg-gray-50 rounded-2xl border border-gray-200">
                ...
                <Link to="/parent" className="mt-6 inline-block text-[#0036a1] font-bold underline">Go Back Home</Link>
            </div>
        </div>
    );
        ...
        <div className="w-full max-w-6xl mx-auto space-y-6">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
                {children.map(c => (
                    <button key={c.id} onClick={() => setSelected(c.id)}
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${selected === c.id ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-500/20' : 'border-slate-200 bg-white hover:bg-slate-50'} shadow-sm`}>
            ...
            {child && (
                <motion.div key={child.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    {/* Profile banner */}
                    <Card className="p-6 bg-gradient-to-r from-blue-700 via-blue-800 to-indigo-900 text-white shadow-lg overflow-hidden relative">
                        ...
                        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <p className="text-blue-200 text-sm">{child.admNo} · {child.class}</p>
                                    <p className="text-blue-200 text-sm">Gender: {child.gender}</p>
                                </div>
                            </div>
                            <div className="flex gap-6">
                                {[
                                    ...
                                    <div key={k.label} className="text-center">
                                        ...
                                        <p className="text-blue-200 text-xs uppercase tracking-wider">{k.label}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        {[
                            { icon: <BookOpen className="w-5 h-5" />, color: 'bg-blue-100 text-blue-700', label: 'Subjects Enrolled', value: child.subjects },
                    ...
                    <Card className="p-5 bg-white border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Link to="/parent/results" className="text-xs text-blue-600 font-semibold hover:underline flex items-center gap-1">
                                View Full Report <ChevronRight className="w-3 h-3" />
                            </Link>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="flex-1 bg-slate-100 rounded-full h-3 overflow-hidden">
                                <div className="bg-blue-600 h-full rounded-full transition-all duration-500" style={{ width: `${child.avg}%` }} />
                            </div>
                            <span className="font-bold text-blue-700 text-lg">{child.avg}%</span>
