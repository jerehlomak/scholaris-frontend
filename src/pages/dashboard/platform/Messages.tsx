        ...
        <div className="max-w-[1200px] mx-auto w-full font-dash pb-4 md:pb-10 px-2 md:px-0" style={{ minHeight: '80vh' }}>
            ...
            <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-3 ${showChat ? 'hidden md:flex' : 'flex'}`}>
                ...
                <button onClick={() => setShowNew(true)}
                    ...
                    style={{ background: 'linear-gradient(135deg, #1E4DA6, #4f46e5)' }}>
            ...
            <div className="flex gap-0 md:gap-5 h-[calc(100vh-180px)] md:h-[65vh]">
                ...
                <div className={`${showChat ? 'hidden md:flex' : 'flex'} w-full md:w-72 md:flex-shrink-0 bg-white rounded-none md:rounded-2xl border-0 md:border border-gray-100 shadow-sm flex-col overflow-hidden`}>
                    <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
                        <MessageSquare className="w-4 h-4 text-[#1E4DA6]" />
                        <span className="text-sm font-bold text-gray-800">Conversations</span>
                    </div>
                    <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
                        {loading ? (
                            <div className="flex items-center justify-center py-12">
                                <div className="w-6 h-6 border-2 border-[#1E4DA6] border-t-transparent rounded-full animate-spin" />
                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-center px-4 gap-2">
                                ...
                                <button onClick={() => setShowNew(true)}
                                    className="text-xs font-semibold text-[#1E4DA6] hover:underline">
                                    Start a conversation
                                </button>
                            </div>
                        ) : conversations.map(conv => (
                            <button key={conv.id} onClick={() => openConversation(conv)}
                                className={`w-full text-left px-4 py-3.5 transition-colors hover:bg-[#1E4DA6]/8 active:bg-[#1E4DA6]/8 ${activeConv?.id === conv.id ? 'bg-[#1E4DA6]/5 border-l-2 border-[#1E4DA6]' : ''}`}>
                                <div className="flex items-start justify-between gap-2 mb-1">
                                    <span className="text-xs md:text-sm font-bold text-gray-800 truncate flex-1">{conv.subject}</span>
                                    {conv.unreadCount > 0 && (
                                        <span className="flex-shrink-0 text-[10px] font-bold text-white bg-[#1E4DA6] rounded-full px-1.5 py-0.5">
                                            {conv.unreadCount}
                                        </span>
                                    )}
                                </div>
                    ...
                    {!activeConv ? (
                        ...
                        <>
                            ...
                            <div className="px-3 md:px-5 py-3.5 border-b border-gray-100 flex items-center justify-between"
                                style={{ background: 'linear-gradient(135deg, #1E4DA6 0%, #4f46e5 100%)' }}>
                                <div className="flex items-center gap-3 flex-1">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[11px] text-white/70 mt-0.5">Platform Support Thread</p>
                                    </div>
                                </div>
                                {activeConv.isClosed && (
                                    ...
                                    return (
                                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[85%] md:max-w-[70%] rounded-2xl px-3 md:px-4 py-2.5 md:py-3 text-sm shadow-sm ${isMe
                                                ...
                                                style={isMe ? { background: 'linear-gradient(135deg, #1E4DA6, #4f46e5)' } : {}}>
                                                {!isMe && (
                                                    <p className="text-[10px] font-bold text-[#1E4DA6] mb-1">
                                                        Skooly Support
                                                    </p>
                                                )}
                                                ...
                                                <div className={`flex items-center gap-1 mt-1.5 ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                    ...
                                                    {isMe && msg.readAt && <CheckCheck className="w-3 h-3 text-white/70" />}
                                                </div>
                                            </div>
                                        </div>
                                    );
                            ...
                            {!activeConv.isClosed ? (
                                <div className="px-3 md:px-4 py-3 border-t border-gray-100 flex items-end gap-2 md:gap-3 bg-white">
                                    <textarea
                                        ...
                                        rows={1}
                                        className="flex-1 resize-none rounded-xl px-3 md:px-4 py-2 md:py-2.5 text-sm outline-none border border-gray-200 bg-gray-50 focus:border-[#1E4DA6] focus:bg-white transition-colors"
                                    />
                    ...
                    <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between"
                            ...
                            <div>
                                ...
                                <p className="text-xs text-white/70 mt-0.5">The support team will receive and reply shortly.</p>
                            </div>
                        ...
                        <form onSubmit={startConversation} className="p-6 space-y-4">
                            <div>
                                ...
                                <input type="text" value={newForm.subject}
                                    ...
                                    className="w-full px-4 py-2.5 rounded-xl text-sm outline-none border border-gray-200 focus:border-[#1E4DA6] transition-colors" />
                            </div>
                            <div>
                                ...
                                <textarea required rows={5} value={newForm.content}
                                    ...
                                    className="w-full px-4 py-2.5 rounded-xl text-sm outline-none border border-gray-200 focus:border-[#1E4DA6] transition-colors resize-none" />
                            </div>
