...
const fieldCls = 'w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm font-medium text-slate-800 shadow-sm transition-all outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100 placeholder:text-slate-400';
...
export default function Expenses() {
    ...
    const fetchExpensesData = async () => {
        try {
            const [ledRes, catRes] = await Promise.all([
                axios.get('/api/v1/finance-v2/ledger?type=EXPENSE', { withCredentials: true }),
                axios.get('/api/v1/finance-v2/categories?type=EXPENSE', { withCredentials: true })
            ]);
    ...
    const handleAdd = async () => {
        try {
            ...
            toast.error(error.response?.data?.msg || 'Failed to add expense record');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Are you sure you want to delete this manual expense record? This action cannot be undone.')) return;
        try {
            await axios.delete(`/api/v1/finance-v2/ledger/EXPENSE/${id}`, { withCredentials: true });
    ...
    if (loading) {
        return (
            <div className="flex min-h-[60vh] flex-col items-center justify-center gap-3">
                ...
                <p className="text-sm font-medium text-slate-500">Loading expense records...</p>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-rose-50/20 to-orange-50/30 px-4 pb-20 pt-8 sm:px-6 lg:px-8">
            <div className="relative z-10 mx-auto max-w-6xl space-y-6">
                ...
                <div className={cn('flex items-center gap-1.5 transition-all duration-500', pageVisible ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0')}>
                    ...
                    <ChevronRight className="h-3 w-3 text-slate-400" />
                    <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-rose-600">Expenses</span>
                </div>
                    ...
                    <div className="p-6 sm:p-8">
                        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-start gap-4">
                                <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-600 to-red-500 shadow-lg shadow-rose-200 text-white">
                                    <TrendingDown className="h-7 w-7" />
                                </div>
                                <div>
                                    <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl">Expense Records</h1>
                                    <p className="mt-1 text-sm leading-relaxed text-slate-500">
                                        Track, filter, and audit operational spending, payroll expenses, and outgoings.
                                    </p>
                                </div>
                            </div>
                            <div className="flex flex-wrap items-center gap-2.5 sm:self-center">
                                ...
                                >
                                    <RefreshCw className={cn('h-3.5 w-3.5', refreshing && 'animate-spin text-rose-600')} />
                                    <span>Refresh</span>
                                </Button>
                                <Button
                                    variant="outline"
                                    onClick={() => mobileSafePrint('expenses-print-area')}
                                    className="h-10 gap-2 rounded-xl border-slate-200 bg-white px-3.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 shadow-sm"
                                >
                                ...
                                <Button
                                    ...
                                    className="h-10 gap-2 rounded-xl bg-rose-600 px-5 text-xs font-bold text-white shadow-md shadow-rose-200 hover:bg-rose-700 transition-all"
                                >
                        ...
                        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div className="rounded-2xl border border-rose-100 bg-gradient-to-br from-rose-500/10 via-rose-50/50 to-white p-5 shadow-sm">
                                <div className="flex items-center justify-between">
                                    <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-rose-700">Total Expenses</span>
                                    <span className="rounded-md bg-rose-100 px-2 py-0.5 font-mono text-[10px] font-bold text-rose-800">{metrics.totalCount} records</span>
                                </div>
                                <p className="mt-2 text-2xl font-black text-slate-900">{fmt(metrics.totalAmount)}</p>
                                <p className="mt-1 text-xs text-slate-500">Net spending volume in current selection</p>
                            </div>
                            ...
                            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                                ...
                                <p className="mt-2 text-2xl font-black text-rose-600">{fmt(metrics.autoAmount)}</p>
                                <p className="mt-1 text-xs text-slate-400">
                                    {metrics.totalAmount > 0 ? `${((metrics.autoAmount / metrics.totalAmount) * 100).toFixed(1)}% of total expenses` : '0% of total'}
                                </p>
                            </div>
                            ...
                            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                                ...
                                <p className="mt-1 text-xs text-slate-400">
                                    {metrics.totalAmount > 0 ? `${((metrics.manualAmount / metrics.totalAmount) * 100).toFixed(1)}% of total expenses` : '0% of total'}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder="Search by staff, payroll, item, utility, reference..."
                                className="w-full rounded-xl border border-slate-200 bg-slate-50/50 pl-10 pr-9 py-2.5 text-sm font-medium text-slate-800 outline-none transition-all focus:border-rose-500 focus:bg-white focus:ring-2 focus:ring-rose-100"
                            />
                            {searchQuery && (
                                <button
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <select
                                ...
                                className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100 shadow-sm"
                            >
                            ...
                            >
                                ...
                                <option value="AUTO">Automated (Payroll)</option>
                        ...
                        <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-slate-100">
                            <div className="flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium outline-none focus:border-rose-500"
                                />
                            </div>
                            <div className="flex items-center gap-2">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                                ...
                                <tr>
                                    ...
                                    >
                                        <div className="flex items-center gap-1.5">
                                            <span>Date</span>
                                            {sortBy === 'date' ? (sortOrder === 'asc' ? <ArrowUp className="h-3 w-3 text-rose-600" /> : <ArrowDown className="h-3 w-3 text-rose-600" />) : <ArrowUpDown className="h-3 w-3 opacity-40" />}
                                        </div>
                                    </th>
                                    <th
                                    ...
                                    >
                                        <div className="flex items-center gap-1.5">
                                            <span>Expense Details & Breakdown</span>
                                            {sortBy === 'description' ? (sortOrder === 'asc' ? <ArrowUp className="h-3 w-3 text-rose-600" /> : <ArrowDown className="h-3 w-3 text-rose-600" />) : <ArrowUpDown className="h-3 w-3 opacity-40" />}
                                        </div>
                                    </th>
                                    <th
                                    ...
                                    >
                                        <div className="flex items-center gap-1.5">
                                            <span>Category</span>
                                            {sortBy === 'category' ? (sortOrder === 'asc' ? <ArrowUp className="h-3 w-3 text-rose-600" /> : <ArrowDown className="h-3 w-3 text-rose-600" />) : <ArrowUpDown className="h-3 w-3 opacity-40" />}
                                        </div>
                                    </th>
                                    ...
                                    >
                                        <div className="flex items-center justify-end gap-1.5">
                                            <span>Amount</span>
                                            {sortBy === 'amount' ? (sortOrder === 'asc' ? <ArrowUp className="h-3 w-3 text-rose-600" /> : <ArrowDown className="h-3 w-3 text-rose-600" />) : <ArrowUpDown className="h-3 w-3 opacity-40" />}
                                        </div>
                                    </th>
                                ...
                                <AnimatePresence>
                                    {filteredRecords.map((t) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-4">
                                                {t.source === 'AUTO' ? (
                                                    <span className="inline-flex items-center gap-1 rounded-md bg-rose-50 px-2 py-0.5 font-mono text-[10px] font-bold text-rose-700 border border-rose-200/60">
                                            ...
                                            <td className="px-5 py-4 text-right font-mono text-sm font-black text-rose-600 whitespace-nowrap">
                                                -{fmt(t.amount)}
                                            </td>
                        ...
                        {filteredRecords.map(t => (
                            <div key={t.id} className="p-4 space-y-2">
                                <div className="flex items-start justify-between gap-2">
                                    ...
                                    <p className="font-mono text-sm font-black text-rose-600 shrink-0">-{fmt(t.amount)}</p>
                                </div>
                    ...
                    {filteredRecords.length === 0 && (
                        <div className="py-16 text-center">
                            ...
                            <p className="mt-3 font-semibold text-slate-700">No expense records matching your search</p>
                    ...
                    <Modal onClose={() => setSelectedRecord(null)} maxWidth="max-w-lg">
                        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/80 px-6 py-4">
                            <div className="flex items-center gap-2.5">
                                ...
                                <span className="font-bold text-slate-900">Expense Record Details</span>
                            </div>
                        ...
                        <div className="space-y-4 p-6 text-sm">
                            <div className="rounded-xl border border-rose-100 bg-rose-50/40 p-4 text-center">
                                <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-rose-700">Expense Outflow</span>
                                <p className="mt-1 font-mono text-3xl font-black text-rose-600">-{fmt(selectedRecord.amount)}</p>
                            </div>
                    ...
                    <Modal onClose={() => !submitting && !creatingCat && setShowForm(false)}>
                        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/80 px-6 py-4">
                            <div className="flex items-center gap-2.5">
                                ...
                                <span className="font-bold text-slate-900">Record Manual Expense</span>
                            </div>
                            <button
                        ...
                        <div className="space-y-4 p-6">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. Science Lab Reagents / Generator Diesel"
                                    className={fieldCls}
                                />
                            </div>
                                    ...
                                    {showNewCatInput ? (
                                        <div className="flex items-center gap-1.5">
                                            <input
                                                ...
                                                className="w-full rounded-xl border border-rose-300 bg-white px-2.5 py-2 text-xs font-medium outline-none focus:ring-2 focus:ring-rose-100"
                                                autoFocus
                                            />
                                            <Button
                                                type="button"
                                                ...
                                                className="h-8 rounded-lg bg-rose-600 px-2.5 text-xs font-bold text-white hover:bg-rose-700"
                                            >
                        ...
                        <div className="flex gap-2 border-t border-slate-100 bg-slate-50/50 px-6 py-4">
                            ...
                            <Button
                                ...
                                className="flex-1 gap-2 rounded-xl bg-rose-600 text-xs font-bold text-white hover:bg-rose-700 shadow-md shadow-rose-200 disabled:opacity-60 transition-all"
                            >
                                {submitting ? (
                                    <>
                                        <Loader2 className="h-4 w-4 animate-spin" />
                                        <span>Saving Expense Record...</span>
                                    </>
                                ) : (
