...
<p>Dear Candidate,</p>
<p><br></p>
<p>Congratulations! We are pleased to offer you the position at our institution.</p>
<p>Please review the details in your portal and accept the offer to proceed with your onboarding.</p>
<p><br></p>
<p>Best Regards,</p>
`;
...
export function EmploymentLetterSetup() {
    ...
    useEffect(() => {
        axios.get('/api/v1/school-settings', { withCredentials: true })
            .then(r => {
                if (r.data.settings?.employmentLetterTemplate) {
                    setTemplate({ ...DEFAULT_TEMPLATE, ...r.data.settings.employmentLetterTemplate });
                }
            })
            .catch(() => {})
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            await axios.patch('/api/v1/school-settings', { employmentLetterTemplate: template }, { withCredentials: true });
            setSaved(true);
    ...
    return (
        <SettingsShell breadcrumbCurrent="Employment Letter" tabLabel="Employment Letter" tabIcon={<Briefcase className="h-3.5 w-3.5" />}>
            <SettingsHero
                icon={<Briefcase className="h-7 w-7" />}
                title="Employment Offer Letter Template"
                subtitle="Configure the official employment offer letter that successful candidates will be able to download from their portal."
            />
